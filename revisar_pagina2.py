...
# Extraer IDs del mapping con sus líneas (PÁGINA 2: líneas 15-84 aproximadamente)
mapping_dict = {}
for i, line in enumerate(mapping_lines, 1):
    # Solo líneas de la página 2 (basado en comentarios)
    if i >= 15 and i <= 84:
...
# Extraer IDs del HTML de la página 2 (líneas 40-152 aproximadamente)
html_dict = {}
for i, line in enumerate(html_lines, 1):
    # Solo líneas de la página 2
    if i >= 40 and i <= 152:
...
# Obtener todos los IDs únicos de la página 2
all_ids = sorted(set(mapping_dict.keys()) | set(html_dict.keys()))

# Crear tabla
print("=" * 180)
print("REVISION PAGINA 2: NIVEL 13.8kV - TRANSFORMADORES")
print("=" * 180)
...
if not solo_mapping and not solo_html:
    print("\n¡Perfecto! Todos los IDs de la PAGINA 2 coinciden.")
