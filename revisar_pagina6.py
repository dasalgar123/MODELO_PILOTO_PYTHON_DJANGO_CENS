...
# Extraer IDs del mapping con sus líneas (PÁGINA 6: líneas 204-240 aproximadamente)
mapping_dict = {}
for i, line in enumerate(mapping_lines, 1):
    # Solo líneas de la página 6 (basado en comentarios)
    if i >= 204 and i <= 240:
...
# Extraer IDs del HTML de la página 6 (líneas 411-481 aproximadamente)
html_dict = {}
for i, line in enumerate(html_lines, 1):
    # Solo líneas de la página 6
    if i >= 411 and i <= 481:
...
# Obtener todos los IDs únicos de la página 6
all_ids = sorted(set(mapping_dict.keys()) | set(html_dict.keys()))

# Crear tabla
print("=" * 180)
print("REVISION PAGINA 6: INTERRUPTOR DE 13.8KV - MODULO DE COMUNICACIONES Y GABINETE")
print("=" * 180)
...
if not solo_mapping and not solo_html:
    print("\n¡Perfecto! Todos los IDs de la PAGINA 6 coinciden.")
