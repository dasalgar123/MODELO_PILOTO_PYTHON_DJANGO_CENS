...
# Extraer IDs del mapping con sus líneas (PÁGINA 5: líneas 165-200 aproximadamente)
mapping_dict = {}
for i, line in enumerate(mapping_lines, 1):
    # Solo líneas de la página 5 (basado en comentarios)
    if i >= 165 and i <= 200:
...
# Extraer IDs del HTML de la página 5 (líneas 320-409 aproximadamente)
html_dict = {}
for i, line in enumerate(html_lines, 1):
    # Solo líneas de la página 5
    if i >= 320 and i <= 409:
...
# Obtener todos los IDs únicos de la página 5
all_ids = sorted(set(mapping_dict.keys()) | set(html_dict.keys()))

# Crear tabla
print("=" * 180)
print("REVISION PAGINA 5: TRANSFORMADOR 34.5/13.8kV - RECONECTADORES")
print("=" * 180)
...
if not solo_mapping and not solo_html:
    print("\n¡Perfecto! Todos los IDs de la PAGINA 5 coinciden.")
