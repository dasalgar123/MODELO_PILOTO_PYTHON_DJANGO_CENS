...
# Extraer IDs del mapping con sus líneas (PÁGINA 8: líneas 264-281 aproximadamente)
mapping_dict = {}
for i, line in enumerate(mapping_lines, 1):
    # Solo líneas de la página 8 (basado en comentarios)
    if i >= 264 and i <= 281:
...
# Extraer IDs del HTML de la página 8 (líneas 539-590 aproximadamente)
html_dict = {}
for i, line in enumerate(html_lines, 1):
    # Solo líneas de la página 8
    if i >= 539 and i <= 590:
...
# Obtener todos los IDs únicos de la página 8
all_ids = sorted(set(mapping_dict.keys()) | set(html_dict.keys()))

# Crear tabla
print("=" * 180)
print("REVISION PAGINA 8: SISTEMA FOTO-VOLTAICO")
print("=" * 180)
...
if not solo_mapping and not solo_html:
    print("\n¡Perfecto! Todos los IDs de la PAGINA 8 coinciden.")
