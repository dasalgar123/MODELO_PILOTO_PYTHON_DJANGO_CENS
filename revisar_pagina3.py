...
# Extraer IDs del mapping con sus líneas (PÁGINA 3: líneas 89-121 aproximadamente)
mapping_dict = {}
for i, line in enumerate(mapping_lines, 1):
    # Solo líneas de la página 3 (basado en comentarios)
    if i >= 89 and i <= 121:
...
# Extraer IDs del HTML de la página 3 (líneas 154-229 aproximadamente)
html_dict = {}
for i, line in enumerate(html_lines, 1):
    # Solo líneas de la página 3
    if i >= 154 and i <= 229:
...
# Obtener todos los IDs únicos de la página 3
all_ids = sorted(set(mapping_dict.keys()) | set(html_dict.keys()))

# Crear tabla
print("=" * 180)
print("REVISION PAGINA 3: NIVEL 13.8kV - TRANSFORMADOR 34.5/13.8KV")
print("=" * 180)
...
if not solo_mapping and not solo_html:
    print("\n¡Perfecto! Todos los IDs de la PAGINA 3 coinciden.")
